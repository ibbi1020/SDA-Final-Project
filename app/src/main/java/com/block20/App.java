...
public class App extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        AuditRepository auditRepo = new SqliteAuditRepository(); // <--- SQL
        NotificationRepository notifRepo = new SqliteNotificationRepository(); // <--- SQL

        // --- STILL IN MEMORY (Partner's Domain) ---
        ...
        this.trainerScheduleService = new TrainerScheduleServiceImpl(trainerService, trainerAvailabilityRepo,
                trainingSessionRepo);

        try {
            // 1. The Student (Islamabad)
            memberService.registerMember(
                    "Bilal Ahmed",
                    "bilal.ahmed@nust.edu.pk",
                    "300-123-4567",
                    "Student",
            ...
            // 2. The Professional (Lahore)
            memberService.registerMember(
                    "Ayesha Malik",
                    "ayesha.m@gmail.com",
                    "321-555-7890",
                    "Premium",
                    "House 45, Block Z, DHA Phase 6, Lahore",
                    "Saad Malik",
                    "321-555-0000",
                    "Spouse");

            // 3. The Bodybuilder (Karachi)
            memberService.registerMember(
                    "Hamza Khan",
                    "hamza.beast@hotmail.com",
                    "333-444-5555",
                    "Elite",
                    "Apartment 4B, Sea View Apartments, Clifton, Karachi",
                    "Kamran Khan",
                    "333-111-2222",
                    "Brother");

            // 4. The Regular (Rawalpindi)
            memberService.registerMember(
                    "Zainab Bibi",
                    "zainab.b@outlook.com",
                    "345-678-9012",
                    "Basic",
            ...
            System.out.println("✅ Successfully added 4 mock members.");
    ...
    private void seedTrainerData() {
        try {
            if (trainerService.getAllTrainers().isEmpty()) {
                trainerService.registerTrainer("Mike", "Johnson", "mike.johnson@block20gym.com", "555-111-2222",
                        "Personal Training", "ACE-CPT", java.time.LocalDate.of(2022, 1, 15), "Lead strength coach");
                trainerService.registerTrainer("Sarah", "Williams", "sarah.williams@block20gym.com", "555-333-4444",
                        "Yoga", "RYT-200", java.time.LocalDate.of(2021, 6, 20), "Morning yoga specialist");
                trainerService.registerTrainer("David", "Chen", "david.chen@block20gym.com", "555-555-6666", "CrossFit",
                        "CF-L2", java.time.LocalDate.of(2020, 3, 10), "CrossFit and HIIT");
            }
        } catch (Exception e) {
    ...
    private void seedTrainerScheduleData() {
        ...
        try {
            ...
            java.util.List<java.time.DayOfWeek> defaultDays = java.util.List.of(java.time.DayOfWeek.MONDAY,
                    java.time.DayOfWeek.WEDNESDAY, java.time.DayOfWeek.FRIDAY);
            for (com.block20.models.Trainer trainer : trainers) {
                ...
                for (java.time.DayOfWeek day : defaultDays) {
                    trainerScheduleService.addAvailabilitySlot(trainer.getTrainerId(), day,
                            java.time.LocalTime.of(8, 0), java.time.LocalTime.of(12, 0));
                    trainerScheduleService.addAvailabilitySlot(trainer.getTrainerId(), day,
                            java.time.LocalTime.of(13, 0), java.time.LocalTime.of(18, 0));
                }
            }

            if (!trainers.isEmpty()) {
                java.time.LocalDate today = java.time.LocalDate.now();
                java.time.LocalDate nextMonday = today
                        .with(java.time.temporal.TemporalAdjusters.nextOrSame(java.time.DayOfWeek.MONDAY));
                java.time.LocalDate nextWednesday = today
                        .with(java.time.temporal.TemporalAdjusters.nextOrSame(java.time.DayOfWeek.WEDNESDAY));
                java.time.LocalDate nextFriday = today
                        .with(java.time.temporal.TemporalAdjusters.nextOrSame(java.time.DayOfWeek.FRIDAY));

                trainerScheduleService.scheduleSession("M1001", "John Smith", trainers.get(0).getTrainerId(),
                        "Personal Training", nextMonday, java.time.LocalTime.of(9, 0), 60, "Strength assessment");
                trainerScheduleService.scheduleSession("M1002", "Sarah Johnson",
                        trainers.get(1 % trainers.size()).getTrainerId(), "Yoga Session", nextWednesday,
                        java.time.LocalTime.of(10, 0), 60, "Morning flow");
                trainerScheduleService.scheduleSession("M1003", "Mike Chen",
                        trainers.get(2 % trainers.size()).getTrainerId(), "CrossFit Training", nextFriday,
                        java.time.LocalTime.of(11, 0), 45, "WOD prep");
            }
        } catch (Exception e) {
